...
class Species(MutableMapping):
    ...
    def print_unknown_warnings(self, s):
        for key in s:
            if key not in self.YAML_MAIN_FIELDS:
                print(
                    "species_gen.py warning: Unknown field '%s' in species %s"
                    % (key, self["enum"]),
                    file=sys.stderr,
                )
    ...
    def levelup_stats_from_yaml(self, s):
        ...
        if (
            self["levelup_stats"] == empty_set("stat_type")
            and self["levelup_stat_frequency"] < 28
        ):
            print(
                "species_gen.py warning: species %s has empty levelup_stats"
                " but a <28 levelup_stat_frequency." % (self["enum"]),
                file=sys.stderr,
            )
        ...
        if (
            ...
            print(
                "species_gen.py warning: species %s has non-empty"
    ...
    def from_yaml(self, s):
        ...
        if self.starting_species != has_recommended_jobs:
            raise ValueError(
                "recommended_jobs must not be empty (or" " difficulty must be False)"
            )

        # Set attributes
        self["enum"] = validate_string(s["enum"], "enum", "SP_[A-Z_]+$")
        self["monster_name"] = validate_string(s["monster"], "monster", "MONS_[A-Z_]+$")
        ...
        self["mp"] = validate_int_range(s["aptitudes"]["mp_mod"], "mp_mod", -5, 20)
        ...
        self["fake_mutations_long"] = fake_mutations_long(s.get("fake_mutations", []))
        self["fake_mutations_short"] = fake_mutations_short(s.get("fake_mutations", []))
        self["recommended_jobs"] = recommended_jobs(s.get("recommended_jobs", []))
        self.set_recommended_weapons(s.get("recommended_weapons", []))
        self["difficulty"] = difficulty(s.get("difficulty"))
        self["difficulty_priority"] = validate_int_range(
            difficulty_priority(s.get("difficulty_priority", 0)),
            "difficulty_priority",
            0,
            1000,
        )
        self["create_enum"] = validate_bool(s.get("create_enum", True), "create_enum")
...
def size(size):
    ...
    if val not in SIZES:
        raise ValueError(
            "Size %s is invalid, pick one of tiny, little, "
...
def mutations(mut_def):
    ...
    for xl, muts in sorted(mut_def.items()):
        ...
        if not isinstance(muts, dict):
            raise ValueError(
                "Mutation key %s doesn't seem to have a valid "
        ...
        for mut_name, amt in sorted(muts.items()):
            ...
            out.append(
                "{{ {mut_name}, {amt}, {xl} }}".format(
                    mut_name=mut_name, xl=xl, amt=amt
                )
            )
...
def fake_mutations_long(fmut_def):
    return make_list(", ".join(quote(m.get("long")) for m in fmut_def if m.get("long")))


def fake_mutations_short(fmut_def):
    return make_list(
        ", ".join(quote(m.get("short")) for m in fmut_def if m.get("short"))
    )
...
def main():
    ...
    species_data_out_text += load_template(
        args.templatedir, "species-data-deprecated-species.txt"
    )
    ...
    aptitudes_out_text += load_template(
        args.templatedir, "aptitudes-deprecated-species.txt"
    )
    ...
    species_groups_out_text += load_template(
        args.templatedir, "species-groups-header.txt"
    )
    species_groups_out_text += generate_species_groups(species_groups)
    species_groups_out_text += load_template(
        args.templatedir, "species-groups-footer.txt"
    )
