...
bool regeneration_is_inhibited()
{
    if (you.get_mutation_level(MUT_INHIBITED_REGENERATION) == 1
        || (you.undead_state() == US_SEMI_UNDEAD && !you.vampire_alive))
    {
...
int player_movement_speed()
{
    ...
    if ((you.in_water() && !you.can_swim() && _slowed_by_water(env.grid(you.pos())))
...
int player::racial_ac(bool temp) const
{
    ...
    if (!(player_is_shapechanged() && temp))
    {
        ...
        if (species == SP_SLITHERIER_NAGA)
            return 200 * experience_level / 3;
...
vector<mutation_ac_changes> all_mutation_ac_changes = {
    ...
    ,mutation_ac_changes(MUT_VAPOROUS_BODY,          mutation_activity_type::FULL,    {2, 4, 6})
    ,mutation_ac_changes(MUT_FAERIE_SCALES,          mutation_activity_type::FULL,    {6, 6, 6})
};
...
mon_holy_type player::holiness(bool temp) const
{
    ...
    if (is_evil_god(religion)
        || species == SP_DEMONSPAWN || you.undead_state() == US_SEMI_UNDEAD)
    {
        holi |= MH_EVIL;
    }
