...
void stop_delay(bool stop_stair_travel)
{
    ...
    if ((
            !delay->is_stair_travel()
            || stop_stair_travel
               && !crawl_state.free_stair_escape
               && (
...
bool MultidropDelay::invalidated()
{
        ...
        )
    {
        ...
        item_def item = *items[0].item;
        inv_from_item_type(inv, item.base_type);

        if (!(*inv)[items[0].slot].defined())
        {
...
{
    if (!drop_item(inv_from_item_type(items[0].item->base_type), items[0].slot, items[0].quantity))
    {
        you.turn_is_over = false;
        you.time_taken = 0;
    }
